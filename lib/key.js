...
class Key {
  ...
  activate() {
    const { settings: { routes: { locked } }, db, middlewares } = this.app
    ...
    locked.forEach(path => middlewares.mount(path, this.authenticate))
  }
  ...
  async verify(key, path) {
    ...
    if (!scopes.some(scope => this.constructor.granted(path, scope))) {
      throw Error(`invalid key ${ key.slice(0, 3) } scope ${ path }`)
    }
  }
  ...
  async find(key) {
    ...
    const data = await db.keys.findOne(
      { _id: hash },
      {
        projection: {
          _id: 0,
  ...
  buffer(key) {
    const { config: { key: { size, encoding } } } = this.app
  ...
  /**
   * Check if the `path` in the `scope`.
   */
